...
class NewComponent extends HTMLElement {
  ...
  initialize() {
    const btn = window.$('#example1');

    // JQuery
    btn.click(e => {
      console.log('hi made it here');
      console.log(window.bioimagesuiteweb);
      console.log(window.bioimagesuiteweb.BisWebDataObject);
      console.log('-----');
      // console.log(THREE);
      console.log('Help', e);
      e.preventDefault();

      const img = new bisweb.BisWebImage();
      img
        .load(
  ...
  connectedCallback() {
    ...
    const layoutid = this.getAttribute('bis-layoutwidgetid');
    console.log('GHERE YOU GO===');
